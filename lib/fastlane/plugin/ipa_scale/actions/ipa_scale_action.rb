  ...
  module Actions
    ...
    class IpaScaleAction < Action
      def self.run(params)
        ...
        ipa_size = (File.size("#{params[:path_to_ipa]}").to_f / 2**20).round(2)
        if ipa_size >= ipa_limit
          ...
          UI.message(" 🚀   IPA size is safe from your #{ipa_limit}MB limit - #{ipa_size}MB!")
          true
        end
      end
      ...
      def self.details
        "This is meant to be called after your gym step as a CI tool to let you know if your latest changes have made your .ipa larger than your desired limit. There are three checks put in place: 🚦   If you exceed your limit, the build will fail. If you are within 5MB of your limit, we will simply warn you but not force fail the build. Otherwise, we will tell you the .ipa size and nothing else!"
      end
      ...
      def self.available_options
        [
           ...
           FastlaneCore::ConfigItem.new(key: :ipa_limit,
                                        ...
                                        verify_block: proc do |value|
                                           UI.user_error!("#{ipa_limit} is not a string") unless String(value) rescue nil
                                        end)
        ]
      end
      ...
      def self.return_value
        "Returns `true` is the IPA is <= given limit, otherwise returns false."
      end
